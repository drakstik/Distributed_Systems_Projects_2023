...
func RaftControl(prt int, ix int, num int, ch chan int) {
	...
	for alive {
		...
		log.Printf("Deactivating Peer from test")
		rf.Deactivate()
		for alive && !active {
			select {
			case i := <-ch:
...
func (ctrl *Controller) checkTerms() int {
	term := -1
	checkingwithSR := StatusReport{}

	leaderSR := StatusReport{}
	...
	for i := 0; i < ctrl.numPeers; i++ {
		...
		if sr.Term > 0 {
			if term == -1 {
				...
				ctrl.cleanup()
				log.Printf("Peer %d is on term %d Expected: %v", i, sr.Term, term)
				log.Printf("Full Status Current: %v Checking with : %v", sr, checkingwithSR)
				log.Printf("Leader SR := %v", leaderSR)
...
func TestCheckpoint_InitialElection(t *testing.T) {
	...
	fmt.Println("First term check ok")
...
func TestCheckpoint_ReElection(t *testing.T) {
	...
	log.Printf("DISCONNECTING LEADER: %v", leader1)
